...
RSpec.describe RbsMiniMagick::Builders::Builder do
  ...
  context "when mini_magick_version does not support" do
    let(:mini_magick_version) { Gem::Version.new("4.13.0") }
  ...
  it_behaves_like "build expected signature" do
    let(:mini_magick_version) { Gem::Version.new("5.0.0") }
    let(:sig_version) { "5.0" }
  end

  it_behaves_like "build expected signature" do
    let(:mini_magick_version) { Gem::Version.new("5.0.1") }
    let(:sig_version) { "5.0" }
  end
end
